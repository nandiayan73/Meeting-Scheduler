...
const Meeting = require('../models/meeting.model'); // assuming Meeting model
const User = require('../models/user.model');       // assuming User model
const mongoose = require('mongoose');

// Middleware to protect route (example - you might already have this)
...
// Create reusable transporter object using SMTP
  ...
  auth: {
    user: 'godaisingh7@gmail.com',      // your Gmail address
    pass: 'ljrf iwcs mcht kfmd',          // your Gmail app password (not regular password)
  }
});
...
const createMeeting = async (req, res) => {
  ...
  const suggestAvailableSlots = async (req, res) => {
    try {
      ...
      const selectedDate = new Date(date); // Ensure date is in YYYY-MM-DD format
      const meetingStartTime = new Date(startTime); // Assume startTime is a complete ISO string (e.g., '2025-05-27T15:00:00.000Z')
      const meetingEndTime = new Date(endTime); // Same for endTime
      ...
      const existingMeetings = await Meeting.find({
        ...
        startTime: {
          $gte: new Date(selectedDate.setDate(selectedDate.getDate() - 2)), // 2 days before the selected date
        },
        endTime: {
          $lte: new Date(new Date(date).setDate(selectedDate.getDate() + 5)) // 5 days after the selected date
        }
      ...
      const minutesStep = 30; // 30-minute step, you can adjust this
      ...
      for (let dayOffset = 0; dayOffset <= maxDaysToCheck; dayOffset++) {
        ...
        let dayStart = new Date(day.setHours(9, 0, 0, 0)); // Set to 9:00 AM
        let dayEnd = new Date(day.setHours(18, 0, 0, 0)); // Set to 6:00 PM
        ...
        while (dayStart < dayEnd) {
          ...
          dayStart = new Date(dayStart.getTime() + minutesStep * 60000); // Step forward by 30 minutes
        }
  
        if (suggestedSlots.length >= 3) break; // Suggest up to 3 slots
      }
  ...
  const getUserMeetings = async (req, res) => {
    try {
      const userId = req.body.user_id;  // From the authenticated user
      ...
      }).populate('participants.user', 'name email')  // Populate participant info (name, email)
      ...
      return res.status(200).json({
        ...
        meetings: userMeetings.map(meeting => ({
          ...
          participants: meeting.participants.map(participant => ({
            ...
            // status: participant.status // status like 'invited', 'accepted', 'rejected'
          }))
        }))
      });
    } catch (error) {
